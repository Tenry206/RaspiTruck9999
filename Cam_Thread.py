...
# ==========================================
# THREAD 0: DEDICATED CAMERA CAPTURE
# ==========================================
def thread_camera():
    global cam
    ...
    while state.running:
        ...
        if frame is not None:
            # Instantly save the newest frame to the shared memory bank
            state.update_frame(frame)
            
        # We DO NOT put a sleep() here. 
        # picam2.capture_array() automatically waits for the hardware shutter,
        # so this thread will perfectly sync to your camera's hardware FPS (e.g., 60 FPS).
...
def thread_line_follow():
    ...
    while state.running:
        ...
        if frame is None:
            sleep(0.01) # Wait briefly if the camera thread is still booting up
            continue
...
orb = cv2.ORB_create(nfeatures=1000, fastThreshold=15, nlevels=12, scaleFactor=1.2, patchSize=31)
