...
def process_shapes(frame):
    ...
    for cnt in contours:
        ...
        if shape_label != 'Noise':
            if shape_label not in ['qr', 'fingerprint', 'recycle']:
                found_solid_shape = True
            results.append({
                'label': shape_label,
    ...
    if len(results) == 0 or not found_solid_shape:
        ...
        if box_found:
            # 2. Shrink inward slightly
            shrink_kernel = np.ones((8, 8), np.uint8)
            roi_mask = cv2.erode(roi_mask, shrink_kernel, iterations=1)
            
            # 3. Mask the color threshold
            fallback_thresh = cv2.bitwise_and(mask_color, roi_mask)
            fallback_thresh = cv2.morphologyEx(fallback_thresh, cv2.MORPH_CLOSE, kernel)
            
            # 4. Fuse inner pieces (Crucial for QR and Fingerprint)
            fb_initial_contours, _ = cv2.findContours(fallback_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            fb_centers = []
            for cnt in fb_initial_contours:
                if cv2.contourArea(cnt) > 300:
                    M = cv2.moments(cnt)
                    if M['m00'] != 0:
                        fb_centers.append((int(M['m10']/M['m00']), int(M['m01']/M['m00'])))
                        
            for i in range(len(fb_centers)):
                for j in range(i+1, len(fb_centers)):
                    if math.hypot(fb_centers[i][0]-fb_centers[j][0], fb_centers[i][1]-fb_centers[j][1]) < 100:
                        cv2.line(fallback_thresh, fb_centers[i], fb_centers[j], 255, thickness=6)
            
            fb_contours, _ = cv2.findContours(fallback_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            
            fallback_results = []
            for cnt in fb_contours:
                if is_line_like_contour(cnt):
                    continue
                    
                shape_label, ar, area, perim, circ, verts = detect_shape(cnt)
                
                # Check ONLY for the complex symbols!
                if shape_label in ['qr', 'fingerprint', 'recycle']:
            ...
            if len(fallback_results) > 0:
                # We successfully found a complex symbol inside the box!
                # We return the fallback mask so the Debug Window shows exactly what it saw!
                return fallback_results, fallback_thresh
